...
def _ensure_ring_info_for_featmaps(mol) -> None:
    ...
    ------
    RuntimeError
        When ring perception fails (indicates invalid/corrupted molecule).
    """
...
def get_sucos_score(mol_1, mol_2, score_mode=None) -> float:
    """SuCOS pharmacophore + shape protrusion score (reference: SuCOS / Runs-N-Poses).

    Matching the reference implementation which does not silently swallow errors.
    RDKit FeatMap or ShapeProtrudeDist failures propagate to the caller.
    """
...
def mol_from_sdf_path(sdf: Path):
    ...
    Returns ``None`` when RDKit cannot parse the file.  Sanitization and
    ring-perception failures propagate (they indicate corrupted chemistry).
    """
...
def ligand_mol_from_pdb(path: Path):
    ...
    :func:`GetFeaturesForMol` then aborts.  This function attempts two strategies:

    1. Direct sanitization of the largest organic fragment.
    2. Round-trip through SMILES + re-embed a 3D conformer.

    Returns ``None`` only when PDB parsing yields no molecule or no organic fragment.
    Sanitization/embedding failures are logged at WARNING.
    """
    from rdkit import Chem  # noqa: PLC0415
    from rdkit.Chem import AllChem  # noqa: PLC0415
    ...
    try:
        ...
        _LOG.warning(
            "ligand_mol_from_pdb: direct sanitization failed for %s (%s: %s); "
            "attempting SMILES round-trip",
    ...
    if fresh is None:
        _LOG.warning("ligand_mol_from_pdb: SMILES round-trip failed for %s", path)
        return None
...
class IncrementalSkrinjarScorer:
    ...
    def _score_pdb_uncached(self, query_pdb: Path) -> float:
        """Score a query PDB against training structures / ligands.

        Returns NaN when the query ligand cannot be extracted (distinguishable
        from a legitimate 0.0 score).  Chemistry errors from individual
        training pairs are logged at WARNING and skipped.
        """
        q_mol = ligand_mol_from_pdb(query_pdb)
        if q_mol is None:
            _LOG.warning(
                "skrinjar: could not extract ligand mol from %s — returning NaN", query_pdb
            )
            return float("nan")
        ...
        for tpath in self._training_complex_candidates(query_pdb):
            ...
            if t_mol is None:
                _LOG.warning("skrinjar: could not extract ligand mol from training PDB %s", tpath)
                continue
            try:
                ...
                _LOG.warning("skrinjar pair skip (%s): %s", type(exc).__name__, exc)
                continue

        for sdf in self.training_ligand_sdfs:
            if not sdf.is_file():
                _LOG.warning("skrinjar: training ligand SDF not found: %s", sdf)
                continue
            t_mol = mol_from_sdf_path(sdf)
            if t_mol is None:
                _LOG.warning("skrinjar: could not load training ligand from %s", sdf)
                continue
            try:
                su = sucos_shape_after_align(q_mol, t_mol, on_align_fallback=bump)
                best = max(best, su)
            except (ValueError, OSError, RuntimeError) as exc:
                _LOG.warning("skrinjar sdf skip (%s): %s", type(exc).__name__, exc)
                continue
...
def load_parquet_similarity_column(
    ...
    Returns ``None`` when the file has no rows, the column is missing, or the
    value is NaN.  File I/O and pyarrow errors propagate.
    """
