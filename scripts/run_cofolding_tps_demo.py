...
>>>>>>> Stashed changes
    ...
    try:
        model, core, batch, processed_dir, _, boltz_run_dir, network_kwargs = build_boltz_session(
            yaml_path=yaml_path,
            cache=cache,
    ...
    if save_traj_every > 0:
        periodic_extra.append(
            (
                trajectory_checkpoint_callback(work_root, bracket_tag="[TPS]", verbose_log=True),
                save_traj_every,
            )
        )
