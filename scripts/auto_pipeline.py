...
Usage:
    python scripts/auto_pipeline.py --target-steps 12000

    # Resume after interruption (just rerun — it's idempotent):
    python scripts/auto_pipeline.py --target-steps 12000

    # Dry run (check state without launching):
    python scripts/auto_pipeline.py --target-steps 12000 --dry-run
"""
...
cmd = [
    sys.executable, "{TPS_REPO}/scripts/orchestrator.py",
    "--base-dir", "{BASE_DIR}",
    "--target-steps", "{target}",
...
def run_assemble(sandbox_id: str) -> None:
    ...
    print(f"[pipeline] Assembling training NPZ (gamma={GAMMA}, max_log_ratio={MAX_LOG_RATIO})...")
