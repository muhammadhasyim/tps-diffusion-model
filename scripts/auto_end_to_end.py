...
Usage:
    PYTHONUNBUFFERED=1 nohup python -u scripts/auto_end_to_end.py > auto_e2e.log 2>&1 &
"""
...
logw_t = gamma * logw
max_lr = 8.0
log_mean = np.log(np.mean(np.exp(logw_t - logw_t.max()))) + logw_t.max()
logw_clipped = np.minimum(logw_t, log_mean + max_lr)
...
cmd = [sys.executable, "{TPS_REPO}/scripts/train_weighted_dsm.py",
    ...
    "--learning-rate", "1e-5", "--beta", "0", "--gamma", "1.0", "--max-grad-norm", "1.0",
