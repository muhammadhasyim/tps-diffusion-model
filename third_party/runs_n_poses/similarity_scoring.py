# Vendored from https://github.com/plinder-org/runs-n-poses
# Upstream commit: 197fafc60a5cef3a9e7f8a4b0dac7c965eed3839
# See README.vendor.md and ../../THIRD_PARTY_NOTICES.md. Document any edits beyond upstream.
...
def get_random_conformer(
    ...
    Args:
        ...
        max_iterations: Maximum number of iterations for conformer generation

    Returns:
        Generated conformer or None if generation fails
    """
...
class SimilarityScorer:
    ...
    def score_system_plinder(self, pdb_id: str) -> None:
        ...
        if not filename.exists():
            LOG.info(f"no foldseek file for {pdb_id}")
            return None
        ...
        scorer.get_score_df(
            PLINDER_DIR, pdb_id=pdb_id, search_db="holo", overwrite=False
        )
    ...
    def score_system_ligand(self, pdb_id: str):
        ...
        system_id_pairs = set(
            zip(scores_df["query_system"], scores_df["target_system"])
        )
        all_systems_to_load = set(scores_df["query_system"]).union(
            set(scores_df["target_system"])
        )
        plindex = query_index(
            columns=[
                "system_id",
        ...
        for system_id, ligand_instance_chain, smiles in tqdm(
            ...
            try:
                ...
                LOG.info(f"failed to get fingerprint for {system_id}: {e}")
                continue
        ...
        for system_1, system_2 in tqdm(system_id_pairs):
            ...
            for ligand_instance_chain_1 in system_id_to_ligand_instance_chain[system_1]:
                ...
                for ligand_instance_chain_2 in system_id_to_ligand_instance_chain[
                    ...
                    sdf_file_1 = PLINDER_DIR / "systems" / system_1 / "ligand_files" / f"{ligand_instance_chain_1}.sdf"
                    sdf_file_2 = PLINDER_DIR / "systems" / system_2 / "ligand_files" / f"{ligand_instance_chain_2}.sdf"
                    ...
                    try:
                        ...
                        try:
                            ...
                            continue
                    try:
                        shape_similarity, color_similarity = align_molecules(
                            mol_1, mol_2
                        )
                    except Exception as e:
            ...
            df = pd.DataFrame([
                {
                    ...
                    "metric": metric,
                    "similarity": np.nanmax(score_dict[metric][(key, system_2)])
        ...
        if len(dfs) > 0:
            ...
            scores_df = (
                pd.merge(
                    scores_df, df, on=["query_system", "target_system"], how="outer"
                )
